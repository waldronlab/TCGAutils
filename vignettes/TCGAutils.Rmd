...
genome(coad[[rag]]) <- translateBuild(genome(coad[[rag]]))
seqlevelsStyle(coad[[rag]]) <- "UCSC"
genome(coad[[rag]])
```
...
```{r}
coad1 <- qreduceTCGA(coad, keep.assay = TRUE)
```
...
```{r}
(xbarcode <- head(colnames(coad1)[["COAD_CNASeq-20160128_simplified"]], 4L))
...
```{r}
# coad2 <- c(coad, COAD_Mutation = re19,
#     mapFrom = "COAD_Mutation-20160128_simplified")
oncoPrintTCGA(coad)
```
