...
coad <- curatedTCGAData::curatedTCGAData(diseaseCode = "COAD",
    assays = c("CNASeq", "Mutation", "miRNA*", "RNASeq2*", "mRNAArray"),
    dry.run = FALSE)
})
```
...
For those rows that cannot be found, an '*_unranged' assay is introduced
...
regions using `qreduceAssay`. The current genome build ('hg18') in the data must
be translated to 'hg19'.

In this example, we first translate the genome strings in the `RaggedExperiment`
`COAD_Mutation-20160128` dataset using `translateBuild`. We do this for the
entire vector by using `vapply`.

```{r}
genome(coad[[4]]) <- vapply(genome(coad[[4]]), translateBuild, character(1L))
seqlevelsStyle(coad[[4]]) <- "UCSC"
genome(coad[[4]])
```
...
tempdir <- tempdir()
gzFile <- file.path(tempdir, basename(loverurl))
download.file(loverurl, destfile = gzFile)
chain <- import.chain(file.path(tempdir, gsub("\\.gz", "", basename(gzFile))))
ranges19 <- liftOver(rowRanges(coad[[4]]), chain)
```
...
process and re-add the metadata assay data to the `RaggedExperiment`.

```{r}
rag <- coad[[4]][as.logical(lengths(ranges19))]
mcor <- mcols(rag)
grl <- relist(unlist(ranges19), rag@assays)
gaga <- unlist(grl, use.names = FALSE)

## add original mcols or assay metadata to all ranges
mcols(gaga) <- mcor
## create new GRangesList from previous data 
newRanges <- relist(gaga, grl)
```

We then convert the `GRangesList` object to `RaggedExperiment` and replace the
old assay with the new one.

```{r}
library(RaggedExperiment)
coad[[4]] <- RaggedExperiment(newRanges)
```


Now that we have matching builds, we can run the `qreduceTCGA` function.
