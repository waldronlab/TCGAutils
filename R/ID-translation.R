...
#'
#' @param slides `logical(1L)` **DEFUNCT**: No longer used. See details.
#'
...
filenameToBarcode <- function(filenames, slides) {
    ...
    if (!missing(slides)) {
        .Defunct(
            msg = "The 'slides' argument is defunct.", package = "TCGAutils"
        )
    }
    filesres <- files()
